        ...
        <div className="flex flex-col h-full">
            ...
            <div className="flex border-b dark:border-white/10 border-black/10 shrink-0 p-2 px-3">
                <button
                    ...
                    className="flex-1 py-2 text-xs font-medium dark:text-white/40 text-gray-400 dark:hover:text-white/60 hover:text-gray-600 transition-colors border-r dark:border-white/10 border-black/10 rounded-none! rounded-l-2xl!"
                >
                    {isCyrillic ? 'Станица' : 'Stanica'}
                </button>
                <button
                    className="flex-1 py-2 text-xs font-medium dark:text-white text-gray-900 rounded-none!"
                >
                    {isCyrillic ? 'Рута' : 'Ruta'}
                </button>
                <button
                    onClick={closeSearch}
                    className="px-4 text-sm dark:text-white/50 text-gray-500 dark:hover:text-white/80 hover:text-gray-800 transition-colors shrink-0 rounded-none! rounded-r-2xl!"
                >
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain px-3 pt-3 pb-8 flex flex-col gap-3">
                ...
                {/* From / To inputs */}
                <div className="rounded-xl dark:bg-white/5 bg-black/5 dark:border-white/10 border-black/10 border overflow-hidden">
                    {/* From row */}
                    <div className="flex items-center gap-2 px-3 py-2.5 border-b dark:border-white/5 border-black/5">
                        ...
                        <input
                            ...
                            className="flex-1 bg-transparent text-sm dark:text-white text-gray-900 placeholder:dark:text-white/30 placeholder:text-gray-400 outline-none min-w-0"
                            autoComplete="off"
                            spellCheck={false}
                        />
                        {fromQuery ? (
                            <button
                                ...
                                className="w-4 h-4 flex items-center justify-center rounded-full dark:bg-white/20 bg-black/10 dark:text-white/60 text-gray-500 text-xs shrink-0"
                                aria-label="Obriši"
                            >
                                ×
                            </button>
                            ...
                            <button
                                ...
                                className="text-xs dark:text-white/40 text-gray-400 shrink-0 dark:hover:text-white/70 hover:text-gray-700 transition-colors"
                    ...
                    {/* Swap button */}
                    <div className="flex items-center justify-center border-b dark:border-white/5 border-black/5 py-0.5">
                        <button
                            onClick={handleSwap}
                            className="w-6 h-6 flex items-center justify-center rounded-full dark:hover:bg-white/10 hover:bg-black/10 transition-colors dark:text-white/40 text-gray-400 dark:hover:text-white/70 hover:text-gray-700 text-sm"
                ...
                {(showFromDropdown || showToDropdown) && (
                    <div className="flex flex-col gap-1 rounded-xl dark:bg-white/5 bg-black/5 dark:border-white/10 border-black/10 border overflow-hidden">
                        {(showFromDropdown ? filteredFrom : filteredTo).map((stop) => (
                            <button
                                key={stop.locationId}
                                onClick={() => showFromDropdown ? handleSelectFrom(stop) : handleSelectTo(stop)}
                                className="flex items-center justify-between gap-3 px-3 py-2.5 text-left w-full dark:hover:bg-white/10 hover:bg-black/8 transition-colors border-b last:border-b-0 dark:border-white/5 border-black/5"
                            >
                                <span className="text-sm dark:text-white text-gray-900 truncate flex-1">
                ...
                {!tripLoading && !tripError && tripResults.length > 0 && (
                    <div className="flex flex-col gap-2">
                        <p className="text-xs font-medium uppercase tracking-wide dark:text-white/40 text-gray-400">
                            {isCyrillic ? 'Резултати' : 'Rezultati'}
                        </p>
                        {tripResults.map((item, i) => (
                            <div
                                key={i}
                                className="rounded-xl dark:bg-white/5 bg-black/5 dark:border-white/10 border-black/10 border p-3 flex flex-col gap-2"
                            >
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-sm font-medium dark:text-white text-gray-900 truncate flex-1">
                                ...
                                <div className="flex items-center gap-1.5 text-xs dark:text-white/60 text-gray-600">
                                    ...
                                    <span className="shrink-0 dark:text-white/30 text-gray-400">→</span>
                                    <span className="truncate">
                                ...
                                <button
                                    ...
                                    className="mt-0.5 w-full py-1.5 rounded-lg text-xs font-medium dark:bg-white/8 bg-black/8 dark:text-white/70 text-gray-600 dark:hover:bg-white/12 hover:bg-black/12 transition-colors"
                                >
